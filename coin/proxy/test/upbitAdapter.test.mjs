...
import { normalizeCandles, normalizeMarkets } from '../src/upbitAdapter.mjs'

test('normalizeMarkets filters quote markets and sorts by volume', () => {
  const markets = [
  ...
  assert.deepEqual(normalizeMarkets(markets, tickers, 'KRW'), [
    {
      market: 'KRW-BTC',
      koreanName: '비트코인',
      englishName: 'Bitcoin',
...
test('normalizeCandles returns ascending chart order', () => {
  const candles = [
    {
      timestamp: 200,
      opening_price: 12,
